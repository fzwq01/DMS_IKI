<template>
    <div id="container">
        <div id="content">
            <span class="tips" v-if="keyIndex <= 5">*请依次插入5个管理员的UKey，用于保存密钥备份文件的保护密钥</span>
            <!-- 完成密钥输入的列表 -->
            ...
            <!-- 表单 -->
            <div class="item" v-if="keyIndex <= 5">
                <div>
                    <div class="UKeyState">
                        <div class="UKeyIcon">
                    ...
                    <el-form :model="uKeyForm" :rules="rules" ref="uKeyForm" label-width="0px" class="demo-ruleForm" v-if="USB">
                        ...
                        <el-form-item class="btnContainer">
                            ...
                            <button class="darkBlueBtn" @click.stop.prevent="submitBackup('uKeyForm')">确定</button>
                        </el-form-item>
                    </el-form>
                </div>
            </div>
        ...
        <div class="btnContainerArea" v-if="keyIndex > 5">
            <button class="cancelBtn" @click.stop.prevent="$emit('cancel')">取 消</button>
            <button class="lightBlueBtn" @click.stop.prevent="install">备 份</button>
        </div>
        <success-tips text="密钥备份成功，您可点击【下载备份】进行密钥备份文件下载" ref="successDialog"></success-tips>
...
import { getSplitKey, getAuthCode, checkManagerUkey, backupSecretKey } from '../../../api/manage/keyManagement' //分割密钥 获取随机数 检查管理员UKey 备份
export default {
    name:'secondStep',
